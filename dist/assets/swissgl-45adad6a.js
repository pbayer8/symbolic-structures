...
`)+`
--- GLSL COMPILE ERROR ---
`+e.getShaderInfoLog(s);e.attachShader(n,s),e.deleteShader(s)}function q(e,t,i){const n=e.createProgram();O(e,t,e.VERTEX_SHADER,n),O(e,i,e.FRAGMENT_SHADER,n),e.linkProgram(n),e.getProgramParameter(n,e.LINK_STATUS)||console.error("shader link error:"+e.getProgramInfoLog(n)),e.useProgram(n),n.setters={},n.samplers=[];const s=e.getProgramParameter(n,e.ACTIVE_UNIFORMS);for(let o=0;o<s;++o){const l=e.getActiveUniform(n,o),d=e.getUniformLocation(n,l.name);if(l.type==e.SAMPLER_2D||l.type==e.SAMPLER_2D_ARRAY)e.uniform1i(d,n.samplers.length),n.samplers.push(l);else{const u=C[l.type];n.setters[l.name]=u.startsWith("uniformMatrix")?h=>e[u](d,!1,h):h=>e[u](d,h)}}return e.useProgram(null),console.log("created",n),n}const I=`
...
#endif
`;function tt(e){const t=[],i={1:"float",2:"vec2",3:"vec3",4:"vec4",9:"mat3",16:"mat4"};for(const n in e){const s=e[n];let o=null;s instanceof WebGLTexture?o=`uniform sampler2D ${n};
...
`)+`
`}const V=e=>e.replace(/\/\*[\s\S]*?\*\/|\/\/.*/g,"");function et(e){e=V(e);const t=Array.from(e.matchAll(/uniform\s+\w+\s+([^;]+)\s*;/g));return new Set(t.map(i=>i[1].split(/[^\w]+/)).flat())}function k(e,t,i){const n=V(e).trim();return n!=""&&n.indexOf(";")==-1&&(e=`${i} = vec4(${n});`),n.match(new RegExp(`\\b${t}s*\\(`))||(e=`void ${t}() {
          ${e};
        }`),e}const it=F(e=>k(e,"vertex","VOut")),nt=F(e=>k(e,"fragment","FOut"));function st(e,t,i,n,s){const o=et([I,i,n,s].join(`
`)),l=Object.entries(t).filter(h=>!o.has(h[0])),d=tt(Object.fromEntries(l)),u=`${I}
${i}
${d}`;return q(e,`
    #define VERT
    ${u}
